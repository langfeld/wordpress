...
class CMB2_REST_Controller_Fields extends CMB2_REST_Controller_Boxes {
	...
	protected function prepare_field_data( CMB2_Field $field ) {
		$field_data = array();
		$parwpml_to_ignore = array( 'show_in_rest', 'options' );
		$parwpml_to_rename = array(
			'label_cb' => 'label',
			'options_cb' => 'options',
		);
		...
		foreach ( $field_args as $key => $value ) {
			if ( in_array( $key, $parwpml_to_ignore, true ) ) {
				continue;
			}
			...
			$key = isset( $parwpml_to_rename[ $key ] ) ? $parwpml_to_rename[ $key ] : $key;
