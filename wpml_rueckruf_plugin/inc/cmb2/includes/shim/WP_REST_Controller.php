...
abstract class WP_REST_Controller {
	...
	/**
	 * Get the query parwpml for collections.
	 *
	 * @return array
	 */
	public function get_collection_parwpml() {
		return array(
	...
	public function get_endpoint_args_for_item_schema( $method = WP_REST_Server::CREATABLE ) {
		...
		foreach ( $schema_properties as $field_id => $parwpml ) {

			// Arguments specified as `readonly` are not allowed to be set.
			if ( ! empty( $parwpml['readonly'] ) ) {
				continue;
			}
			...
			if ( isset( $parwpml['description'] ) ) {
				$endpoint_args[ $field_id ]['description'] = $parwpml['description'];
			}

			if ( WP_REST_Server::CREATABLE === $method && isset( $parwpml['default'] ) ) {
				$endpoint_args[ $field_id ]['default'] = $parwpml['default'];
			}

			if ( WP_REST_Server::CREATABLE === $method && ! empty( $parwpml['required'] ) ) {
				$endpoint_args[ $field_id ]['required'] = true;
			}

			foreach ( array( 'type', 'format', 'enum' ) as $schema_prop ) {
				if ( isset( $parwpml[ $schema_prop ] ) ) {
					$endpoint_args[ $field_id ][ $schema_prop ] = $parwpml[ $schema_prop ];
				}
			}

			// Merge in any options provided by the schema property.
			if ( isset( $parwpml['arg_options'] ) ) {

				// Only use required / default from arg_options on CREATABLE endpoints.
				if ( WP_REST_Server::CREATABLE !== $method ) {
					$parwpml['arg_options'] = array_diff_key( $parwpml['arg_options'], array( 'required' => '', 'default' => '' ) );
				}

				$endpoint_args[ $field_id ] = array_merge( $endpoint_args[ $field_id ], $parwpml['arg_options'] );
			}
		}
